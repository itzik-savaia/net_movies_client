        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.adventure === undefined ? null :
                            categorys.adventure.length >= 7 ?
                                <Fragment key={i}>
                                    <div className="row-header">
                                        <div className="rowTitle">
                                            <div className="row-header-title" key={Object.keys(categorys).find(c => c === "adventure")}>
                                                {Object.keys(categorys).find(c => c === "adventure").toUpperCase().substr(0, 1).toUpperCase() + Object.keys(categorys).find(c => c === "adventure").substr(1)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "adventure")}>
