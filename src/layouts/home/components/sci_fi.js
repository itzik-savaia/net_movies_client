...
class Sci_Fi extends Component {
    constructor(props) {
        super(props);
        this.state = {
            display: true,
        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.sci_Fis === undefined ? null :
                            categorys.sci_Fis.length >= 4 ?
                                <Fragment key={i}>
                                    ...
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "sci_Fis")} >
                                            {categorys.sci_Fis.map(movie => (movie.photos.map(photo => (
                                        ...
                                        {this.call_cart_movie(categorys.sci_Fis)}
                                    </div>
                                </Fragment>
                                : null}
                    </Fragment>
...
export default connect(mapStateToProps)(Sci_Fi);