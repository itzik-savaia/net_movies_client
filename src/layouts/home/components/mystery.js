        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.mystery === undefined ? null :
                            categorys.mystery.length >= 7 ?
                                <Fragment key={i}>
                                    <div className="row-header">
                                        <div className="rowTitle">
                                            <div className="row-header-title" key={Object.keys(categorys).find(c => c === "mystery")}>
                                                {Object.keys(categorys).find(c => c === "mystery").toUpperCase().substr(0, 1).toUpperCase() + Object.keys(categorys).find(c => c === "mystery").substr(1)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "mystery")} >
                                            {categorys.mystery.map(movie => (movie.photos.map(photo => (
                                        ...
                                        {this.call_cart_movie(categorys.mystery)}
                                    </div>
                                </Fragment>
                                : null}
                    </Fragment>
