        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.terrors === undefined ? null :
                            categorys.terrors.length >= 7 ?
                                <Fragment key={i}>
                                    <div className="row-header">
                                        <div className="rowTitle">
                                            <div className="row-header-title" key={Object.keys(categorys).find(c => c === "terrors")}>
                                                {Object.keys(categorys).find(c => c === "terrors").toUpperCase().substr(0, 1).toUpperCase() + Object.keys(categorys).find(c => c === "terrors").substr(1)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "terrors")}>
                                            {categorys.terrors.map(movie => (movie.photos.map(photo => (
                                        ...
                                        {this.call_cart_movie(categorys.terrors)}
                                    </div>
                                </Fragment>
                                : null}
                    </Fragment>
