...
class Comedie extends Component {
    constructor(props) {
        super(props);
        this.state = {
            display: true,
        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.comedies === undefined ? null :
                            categorys.comedies.length >= 7 ?
                                <Fragment key={i}>
                                    <div className="row-header">
                                        <div className="rowTitle">
                                            <div className="row-header-title" key={Object.keys(categorys).find(c => c === "comedies")}>
                                                {Object.keys(categorys).find(c => c === "comedies").toUpperCase().substr(0, 1).toUpperCase() + Object.keys(categorys).find(c => c === "comedies").substr(1)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "comedies")} >
                                            {categorys.comedies.map(movie => (movie.photos.map(photo => (
                                        ...
                                        {this.call_cart_movie(categorys.comedies)}
                                    </div>
                                </Fragment>
                                : null}
...
export default connect(mapStateToProps)(Comedie);