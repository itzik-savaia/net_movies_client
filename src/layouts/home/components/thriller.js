        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.thrillers === undefined ? null :
                            categorys.thrillers.length >= 7 ?
                                <Fragment key={i}>
                                    ...
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "thrillers")} >
                                            {categorys.thrillers.map(movie => (movie.photos.map(photo => (
                                                                    ...
                                                                    <div className="btnBackround">
                                                                        <button className="btn"
                                                                            onClick={() => {
                                                                                this.setState({
                                                                                    find: movie,
                                                                                    find_category: categorys.thrillers
                                                                                })
                                                                            }}> ^
                                                                </button>
                                                                    </div>
                                        ...
                                        {this.call_cart_movie(categorys.thrillers)}
                                    </div>
                                </Fragment>
                                : null}
                    </Fragment>
...
export default connect(mapStateToProps)(Thriller);