...
class Stand_Up_Show extends Component {
    constructor(props) {
        super(props);
        this.state = {
            display: true,
        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.stand_up_shows === undefined ? null :
                            categorys.stand_up_shows.length >= 7 ?
                                <Fragment key={i}>
                                    ...
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "stand_up_shows")} >
                                            {categorys.stand_up_shows.map(movie => (movie.photos.map(photo => (
                                                                    ...
                                                                    <div className="btnBackround">
                                                                        <button className="btn"
                                                                            onClick={() => {
                                                                                this.setState({
                                                                                    find: movie,
                                                                                    find_category: categorys.stand_up_shows
                                                                                })
                                                                            }}> ^
                                                                </button>
                                                                    </div>
                                        ...
                                        {this.call_cart_movie(categorys.stand_up_shows)}
                                    </div>
                                </Fragment>
                                : null}
                    </Fragment>
...
export default connect(mapStateToProps)(Stand_Up_Show);