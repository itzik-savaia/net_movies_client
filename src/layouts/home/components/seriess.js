        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.seriess === undefined ? null :
                            categorys.seriess.length >= 7 ?
                                <Fragment key={i}>
                                    <div className="row-header">
                                        <div className="rowTitle">
                                            <div className="row-header-title" key={Object.keys(categorys).find(c => c === "seriess")}>
                                                {Object.keys(categorys).find(c => c === "seriess").toUpperCase().substr(0, 1).toUpperCase() + Object.keys(categorys).find(c => c === "seriess").substr(1)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "seriess")} >
                                            {categorys.seriess.map(movie => (movie.photos.map(photo => (
                                        ...
                                        {this.call_cart_movie(categorys.seriess)}
                                    </div>
                                </Fragment>
                                : null}
                    </Fragment>
