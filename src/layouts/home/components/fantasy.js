        ...
        return (
            <div>
                {Object.values(this.state.categorys.Movies_Categorys_Reducer).map((categorys, i) => (
                    <Fragment key={i}>
                        {categorys.fantasys === undefined ? null :
                            categorys.fantasys.length >= 7 ?
                                <Fragment key={i}>
                                    <div className="row-header">
                                        <div className="rowTitle">
                                            <div className="row-header-title" key={Object.keys(categorys).find(c => c === "fantasys")}>
                                                {Object.keys(categorys).find(c => c === "fantasys").toUpperCase().substr(0, 1).toUpperCase() + Object.keys(categorys).find(c => c === "fantasys").substr(1)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className='top'>
                                        <Slider {...settings} key={Object.keys(categorys).find(c => c === "fantasys")} >
                                            {categorys.fantasys.map(movie => (movie.photos.map(photo => (
                                        ...
                                        {this.call_cart_movie(categorys.fantasys)}
                                    </div>
                                </Fragment>
                                : null}
                    </Fragment>
